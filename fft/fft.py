import fft.hps_0 as HPS_0
from de0.sgdma import *
from de0.de0mem_py import *

data_length = 128

DATA_BASE = HPS_0.FFT_SUB_DATA.BASE + ALT_LWFPGASLVS
for i in range(0, data_length):
	temp = 0x7fff if ((i % 32) < 16) else 0x8000
	write_uint32_to_pa(DATA_BASE + i * 4, temp)
	print("signal 0x%X %d" % (i, temp))

RESULT_BASE = HPS_0.FFT_SUB_DATA.BASE + ALT_LWFPGASLVS + HPS_0.FFT_SUB_DATA.SPAN // 2
for i in range(0, data_length * 2):
	write_uint32_to_pa(RESULT_BASE + i * 4, 0) # zero out result

# tell the FFT how long the fft is
FFT_CSR_BASE = ALT_LWFPGASLVS + HPS_0.FFT_SUB_FFT_STADAPTER_0.BASE
...
SGDMA_TO_FFT_CSR_BASE = ALT_LWFPGASLVS + HPS_0.FFT_SUB_SGDMA_TO_FFT_CSR.BASE
SGDMA_TO_FFT_DESCRIPTOR_SLAVE_BASE = ALT_LWFPGASLVS + HPS_0.FFT_SUB_SGDMA_TO_FFT_DESCRIPTOR_SLAVE.BASE
descriptor0.write(
	SGDMA_TO_FFT_CSR_BASE,
	SGDMA_TO_FFT_DESCRIPTOR_SLAVE_BASE
)

SGDMA_FROM_FFT_CSR_BASE = ALT_LWFPGASLVS + HPS_0.FFT_SUB_SGDMA_FROM_FFT_CSR.BASE
SGDMA_FROM_FFT_DESCRIPTOR_SLAVE_BASE = ALT_LWFPGASLVS + HPS_0.FFT_SUB_SGDMA_FROM_FFT_DESCRIPTOR_SLAVE.BASE
descriptor1.write(
